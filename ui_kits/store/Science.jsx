    ...
    <section id="ingredients" style={{ background: "var(--cream)" }}>
      <div style={{ maxWidth: "var(--container)", margin: "0 auto", padding: phone ? "var(--section-y-mobile) var(--gutter)" : "var(--section-y) var(--gutter)", display: "grid", gap: phone ? "var(--space-6)" : "var(--space-7)" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: window.cols(phone, "repeat(3,1fr)"), gap: "var(--space-4)" }}>
          ...
          <IngredientCard name={window.copy("ing.b5.name", "Pro-Vitamin B5", "بروفيتامين ب5")} arabic={ar ? "" : "بروفيتامين ب5"} image="../../assets/photography/ingredient-b5.jpg">{window.copy("ing.b5.desc", "Deeply hydrates for smooth, silky hair.", "بيرطّب بعمق لشعر ناعم وحريري.")}</IngredientCard>
          <IngredientCard name={window.copy("ing.e.name", "Vitamin E", "فيتامين هـ")} arabic={ar ? "" : "فيتامين هـ"} image="../../assets/photography/ingredient-vitamin-e.jpg">{window.copy("ing.e.desc", "Antioxidant care for healthy-looking, radiant hair.", "عناية مضادة للأكسدة لشعر صحي ولامع.")}</IngredientCard>
        </div>
      </div>
    </section>
  );
